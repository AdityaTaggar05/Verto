...
Future<List<AvatarItem>?> fetch({required BuildContext context}) async {
  List<AvatarItem>? items = await makeRequest<List<AvatarItem>>(
    type: RequestType.get,
    path: "/api/wardrobe",
    fromJson: (fetched) =>
        fetched.map<AvatarItem>((item) => AvatarItem.fromJson(item)).tolist(),
  );
...
void purchase({required BuildContext context, required String id}) async {
